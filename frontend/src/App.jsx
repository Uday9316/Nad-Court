...
const fetchLiveEvaluation = async (judge, caseData, plaintiffArgs, defendantArgs) => {
  try {
...
function App() {
  ...
  // LIVE API Integration - Real-time AI arguments and judge evaluations
  useEffect(() => {
    if (view !== 'live') return
    ...
    const runLiveCase = async () => {
      ...
      for (let round = 1; round <= 6; round++) {
        ...
        const pArg = await fetchLiveArgument('plaintiff', caseData, round)
        if (pArg) {
        ...
        const dArg = await fetchLiveArgument('defendant', caseData, round)
        if (dArg) {
      ...
      for (const judge of JUDGES) {
        const evalData = await fetchLiveEvaluation(judge.name, caseData, plaintiffArgs, defendantArgs)
        if (evalData) {
